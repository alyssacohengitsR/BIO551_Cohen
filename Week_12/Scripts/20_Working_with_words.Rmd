...
## Types of functions in {stringr} package
...
\
This can be useful if say you have a two columns of treatments and you want to combine them into one (e.g., high temp, low temp and high pH, low pH).
...
Here I am duplicating it 2 and 3 times
```{r}
...
[regex cheat sheet](http://edrub.in/CheatSheets/cheatSheetStringr.pdf)
\
\
...
![](https://d2o2utebsixu4k.cloudfront.net/tutorials/topics/images/1568271038916-image6.jpg)
...
![](https://d2o2utebsixu4k.cloudfront.net/tutorials/topics/images/1568271446941-image8.jpg)
...
test %>%
  ...
  str_replace_all("[a-zA-Z]|\\:", "") %>% # remove the letters and the :
...
OK so we now have >735,000 rows of words.... but, some of these words are kind of useless.  Words that are common and don't really have important meaning (e.g. "and","by","therefore"...).  These are called stopwords.  
We can use the function `get_stopwords()` to essentially remove these words from our dataframe. (This function is essentially just a dataframe of unnecessary words)
...
## count most common words in dataframe
...
How would we modify this code to count the most popular words by book? 
```{r}
cleaned_books %>% 
  group_by(book) %>%
  count(word, sort = TRUE) 
...
What about by each chapter within a book? 
```{r}
cleaned_books %>% 
...
sent_word_counts %>%
  ...
  theme_bw() + 
  theme(legend.key.size = unit(5, "pt"), legend.position = c(0.9,0.1),
...
cleaned_books %>%
  ...
  wordcloud2(shape = 'triangle', size=0.3) # make a wordcloud out of the top 100 words
```
